...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    const findProductsWithQuantityAvailabe = products.filter(
      product =>
        existentProducts.filter(p => p.id === product.id)[0].quantity <
        product.quantity
    );

    if(findProductsWithQuantityAvailabe.length){
      throw new AppError(
        `Tha quantity ${findProductsWithQuantityAvailabe[0].quantity} is not available for ${findProductsWithQuantityAvailabe[0].id}`
      );
    }
